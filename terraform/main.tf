...
resource "aws_iam_role_policy" "wallet_ec2" {
  name = "${var.project}-ec2-policy"
  role = aws_iam_role.wallet_ec2.id
...
resource "aws_iam_instance_profile" "wallet" {
  ...
  role = aws_iam_role.wallet_ec2.name
}

# -----------------------------
# Compute Target (EC2)
resource "aws_instance" "wallet" {
  ami                    = "ami-0c55b159cbfafe1f0" # dummy AMI for LocalStack
